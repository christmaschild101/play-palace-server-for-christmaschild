...
accounts-blocked = Entschuldigung, die Registrierung ist derzeit deaktiviert. Nur Administratoren oder der Serverbesitzer können momentan Konten erstellen. Schau später wieder vorbei!
# Aktionszustände
action-locked = Diese Aktion ist derzeit nicht verfügbar.
action-not-available = Diese Aktion ist gerade nicht verfügbar.
# Gründe für deaktivierte Aktionen
action-player-not-found = Spieler nicht gefunden.
category-playaural = PlayAural
confirm-logout = Bist du sicher, dass du dich abmelden möchtest?
# Dokumente
documents = Dokumente
documents-add-translation = Übersetzung hinzufügen
documents-all = Alle Dokumente
documents-based-on-stale = Upstream-Quelle geändert: { $source }
documents-categories-updated = Kategorien aktualisiert.
documents-category-created = Kategorie erstellt.
documents-category-deleted = Kategorie gelöscht.
documents-category-name-prompt = Gib den Anzeigenamen dieser Kategorie für die Übersetzung { $language } ein:
documents-category-renamed = Kategorie umbenannt.
documents-category-settings = Kategorieeinstellungen
documents-commit-failed = Commit fehlgeschlagen: { $reason }
documents-commit-message-prompt = Beschreibe deine Änderungen (optional):
documents-commit-success = Änderungen committet.
documents-content-label = { $language } Inhalt
documents-content-prompt = Gib den Inhalt für die Übersetzung { $language } ein:
documents-content-saved = Dokumentinhalt für { $language } gespeichert.
documents-content-unchanged = Nichts zu speichern geändert.
documents-delete-category = Kategorie löschen
documents-delete-category-confirm = Diese Kategorie löschen? Dokumente werden nicht gelöscht, verlieren aber diese Kategoriezuordnung.
documents-delete-confirm = Dieses Dokument löschen? Es hat { $count } Übersetzungen. Dies kann nicht rückgängig gemacht werden.
documents-delete-document = Dokument löschen
documents-delete-locked = Dieses Dokument kann nicht gelöscht werden, weil die Übersetzung { $language } gerade von { $username } bearbeitet wird.
documents-deleted = Dokument gelöscht.
documents-document-created = Dokument erstellt.
documents-editor-prompt = Bearbeiten: { $title } ({ $language })
documents-export-no-changes = Keine ausstehenden Änderungen zum Exportieren.
documents-export-pending = Ausstehende Änderungen exportieren ({ $count })
documents-export-success = { $count } Änderungen nach { $path } exportiert.
documents-locked = Dieses Dokument wird gerade von { $username } bearbeitet. Du kannst es momentan nicht bearbeiten.
documents-manage-visibility = Sichtbarkeit verwalten
documents-menu-title = Dokumentensystem
documents-modify-categories = Kategorienliste ändern
documents-new-category = Neue Kategorie
documents-new-category-slug-prompt = Gib den englischen Slug-Namen ein. Dies ist die interne Kennung der Kategorie, zum Beispiel game_rules:
# Dokument- und Kategorieerstellung
documents-new-document = Neues Dokument
documents-new-document-slug-prompt = Gib den englischen Slug-Titel ein. Dies ist die interne Kennung des Dokuments, zum Beispiel my_game_rules:
documents-no-content = Für dieses Dokument ist kein Inhalt verfügbar.
documents-no-documents = Keine Dokumente gefunden.
documents-no-languages-available = Keine Sprachen für die Übersetzung verfügbar.
documents-no-permission = Du hast keine relevanten zugewiesenen Sprachen für dieses Dokument.
documents-pending-commits-button = Ausstehende Commits ({ $count })
documents-pending-commits-info = { $count } Commits vor upstream. Der Serverbesitzer kann diese Änderungen in das Repository pushen.
documents-pr-button = Pull-Request erstellen ({ $count } Commits)
documents-pr-failed = Pull-Request fehlgeschlagen: { $reason }
documents-pr-no-commits = Keine Commits für einen Pull-Request vorhanden.
documents-pr-success = Pull-Request erstellt: { $url }
documents-promote-confirm = Bist du sicher, dass du dieses Dokument in den gemeinsamen Bereich verschieben möchtest? Dadurch wird es allen Servern sichtbar, die aus diesem Repository synchronisieren.
documents-promote-failed = Dokument konnte nicht verschoben werden. Es ist möglicherweise bereits gemeinsam oder es besteht ein Konflikt.
documents-promote-to-shared = In geteilt verschieben
documents-promoted-to-shared = Dokument in den gemeinsamen Bereich verschoben.
documents-remove-title-confirm = Auch den { $language }-Titel entfernen? Wähle Nein, um ihn für eine spätere Übersetzung aufzubewahren.
documents-remove-translation = Übersetzung entfernen
documents-remove-translation-confirm = Die Übersetzung { $language } entfernen? Dies kann nicht rückgängig gemacht werden.
documents-remove-translation-locked = Die Übersetzung { $language } wird gerade von { $username } bearbeitet und kann nicht entfernt werden.
documents-remove-translation-source = Die Quellübersetzung kann nicht entfernt werden.
# Kategorieverwaltung
documents-rename-category = Kategorie umbenennen
documents-scope-independent = Unabhängig (nur dieser Server)
documents-scope-prompt = Wähle den Bereich für dieses Dokument.
documents-scope-shared = Geteilt (alle Server)
documents-select-categories = Wähle Kategorien für das neue Dokument.
documents-settings = Dokumenteinstellungen
documents-slug-exists = Ein Dokument mit dieser Kennung existiert bereits.
documents-slug-exists-category = Eine Kategorie mit diesem Slug existiert bereits.
documents-slug-invalid = Ungültiger Slug. Verwende nur Kleinbuchstaben, Zahlen und Unterstriche.
documents-sort-alphabetical = Alphabetisch
documents-sort-changed = Sortiermethode aktualisiert.
documents-sort-date-created = Erstellungsdatum
documents-sort-date-modified = Änderungsdatum
documents-sort-method = Sortiermethode
documents-source-label = { $language } (Quelle)
# Dokumentinfrastruktur (Synchronisierung, Export, Verschieben, Beitrag)
documents-sync = Gemeinsame Dokumente synchronisieren
documents-sync-confirm = Jetzt synchronisieren
documents-sync-discard-all = Alle verwerfen
documents-sync-discard-label = { $title } (verwerfen); ({ $description })
documents-sync-failed = Synchronisierung fehlgeschlagen: { $reason }
documents-sync-keep-all = Alle behalten
documents-sync-keep-label = { $title } (behalten); ({ $description })
documents-sync-local-changes = { $count } Dokumente haben lokale Änderungen. Wähle vor der Synchronisierung aus, welche verworfen werden sollen.
documents-sync-pending-warning = Warnung: Es gibt { $count } nicht committete Änderungen an gemeinsamen Dokumenten. Exportiere sie vor der Synchronisierung, um lokale Bearbeitungen nicht zu verlieren.
documents-sync-success = Gemeinsame Dokumente erfolgreich synchronisiert.
documents-sync-tag-absent = Element fehlt lokal: behalten = nicht von upstream hinzufügen, verwerfen = von upstream hinzufügen
documents-sync-tag-content = Inhaltsänderungen
documents-sync-tag-content-and-metadata = Inhalts- und Metadatenänderungen
documents-sync-tag-metadata = Metadatenänderungen
documents-sync-tag-present = Element ist nur lokal vorhanden: behalten = nicht lokal löschen, verwerfen = lokal löschen
documents-title-changed = Titel für { $language } aktualisiert.
documents-title-prompt = Gib den Dokumenttitel für die Übersetzung { $language } ein:
documents-translation-added = Übersetzung für { $language } hinzugefügt.
documents-translation-removed = Übersetzung { $language } entfernt.
documents-uncategorized = Nicht kategorisierte Dokumente
documents-update-contents = Dokumentinhalt bearbeiten
documents-update-title = Titel ändern
# Dokumentaktionen
documents-view = Dokumentinhalt anzeigen
documents-visibility-changed = Sichtbarkeit für { $language } aktualisiert.
documents-visibility-count = Sichtbarkeit verwalten ({ $public }/{ $total } Sprachen öffentlich)
documents-visibility-no-permission = Du hast keine Berechtigung, die Sichtbarkeit für { $language } zu ändern.
done = Fertig
fluent-languages-option = Fließende Sprachen ({ $count })
game-over-leave = Herzlichen Glückwunsch, du hast es großartig gemacht!
# Fehler
internal-error = Etwas ist schiefgelaufen. Bitte versuche es erneut.
no-users-to-reset-password = Keine Benutzer für das Zurücksetzen des Passworts verfügbar.
option-locked = Diese Option ist derzeit durch eine andere Einstellung gesperrt.
placeholder-feature = Diese Funktion wird in Betracht gezogen und ist derzeit nicht verfügbar.
# Präferenzsystem
pref-back = Zurück
pref-category-dice = Würfelverhalten
# Präferenzkategorien
pref-category-display = Anzeige
pref-category-gameplay = Spielweise
pref-category-sounds = Geräusche
pref-changed-brief-announcements = Kurze Ansagen { $status }.
pref-changed-clear-kept-on-roll = Beh altene Würfel beim Würfeln löschen { $status }.
pref-changed-confirm-destructive-actions = Destruktive Aktionen bestätigen { $status }.
pref-changed-dice-keeping-style = Würfelhalte-Stil auf { $choice } gesetzt.
pref-changed-play-turn-sound = Zuggeräusch { $status }.
pref-default = Standard
pref-desc-brief-announcements = Verwende kürzere Ansagen während des Spiels statt ausführlichem Kommentar
pref-desc-clear-kept-on-roll = Alle behaltenen Würfel nach jedem Wurf automatisch freigeben
pref-desc-confirm-destructive-actions = Bei destruktiven Aktionen wie dem Überspringen des Zuges um Bestätigung bitten
pref-desc-dice-keeping-style = Wie Würfel zum Behalten ausgewählt werden
pref-desc-play-turn-sound = Ein Geräusch abspielen, wenn du am Zug bist
pref-dice-keeping-style-playpalace = Würfelindizes
pref-dice-keeping-style-quentin_c = Würfelwerte
pref-per-game-for = { $game }: { $value }
pref-reset-all = Alle Präferenzen auf Standard zurücksetzen
pref-reset-category = { $category } auf Standard zurücksetzen
pref-reset-done = Präferenzen auf Standard zurückgesetzt.
pref-select-dice-keeping-style = Würfelhalte-Stil auswählen:
pref-set-brief-announcements = Kurze Ansagen: { $status }
pref-set-clear-kept-on-roll = Beh altene Würfel beim Würfeln löschen: { $status }
pref-set-confirm-destructive-actions = Destruktive Aktionen bestätigen: { $status }
pref-set-dice-keeping-style = Würfelhalte-Stil: { $choice }
# Präferenzbeschriftungen (in Menüs)
pref-set-play-turn-sound = Zuggeräusch: { $status }
# Passwortwiederherstellung
reset-user-password = Benutzerpasswort zurücksetzen
reset-user-password-done = Das Passwort von { $player } wurde zurückgesetzt.
reset-user-password-prompt = Gib ein neues temporäres Passwort für { $player } ein (oder drücke Escape zum Abbrechen):
reset-user-password-unavailable = { $player } ist für das Zurücksetzen des Passworts nicht verfügbar.
transcribers-add-languages = Sprachen hinzufügen
transcribers-add-user = Benutzer hinzufügen
transcribers-add-users = Benutzer hinzufügen
transcribers-added = { $user } wurde als Transkribent für { $language } hinzugefügt.
# Transkribentenverwaltung
transcribers-by-language = Transkribenten nach Sprache anzeigen
transcribers-by-user = Transkribenten nach Benutzer anzeigen
transcribers-language-users = { $language } ({ $count } Benutzer)
transcribers-language-users-one = { $language } ({ $count } Benutzer)
transcribers-languages-added = { $user } als Transkribent für { $languages } hinzugefügt.
transcribers-no-eligible-languages = Keine geeigneten Sprachen gefunden. Dieser Benutzer hat keine nicht zugewiesenen fließenden Sprachen.
transcribers-no-eligible-users = Keine geeigneten Benutzer gefunden. Benutzer müssen diese Sprache in ihren fließenden Sprachen haben.
transcribers-no-languages = Keine Sprachen diesem Benutzer zugewiesen.
transcribers-no-transcribers = Es wurden noch keine Transkribenten zugewiesen.
transcribers-no-users = Keine Transkribenten dieser Sprache zugewiesen.
transcribers-no-users-to-add = Keine Benutzer zum Hinzufügen verfügbar. Alle Benutzer mit fließenden Sprachen sind bereits Transkribenten.
transcribers-not-fluent = { $user } hat { $language } nicht in seinen fließenden Sprachen und kann nicht als Transkribent dafür zugewiesen werden.
transcribers-remove-all-confirm = Alle Transkribentenzuweisungen von { $user } entfernen?
transcribers-remove-confirm = { $user } als Transkribent für { $language } entfernen?
transcribers-remove-lang-confirm = { $language } aus den Transkribentenzuweisungen von { $user } entfernen?
transcribers-remove-transcriber = Transkribent entfernen
transcribers-removed = { $user } wurde als Transkribent für { $language } entfernt.
transcribers-removed-all = Alle Transkribentenzuweisungen von { $user } entfernt.
transcribers-user-languages = { $user } ({ $count } Sprachen)
transcribers-user-languages-one = { $user } ({ $count } Sprache)
transcribers-users-added = { $users } als Transkribenten für { $language } hinzugefügt.
unknown-player = Unbekannter Spieler
visibility-available = verfügbar
visibility-private = privat
# Sichtbarkeitskontrolle
visibility-public = öffentlich
visibility-unavailable = nicht verfügbar
your-password-was-reset = Dein Passwort wurde von einem Administrator zurückgesetzt. Bitte melde dich mit dem neuen Passwort an.
